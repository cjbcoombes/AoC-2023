...
getHPref arr = let ((_, _), (h, w)) = bounds arr
               in reverse $ snd $ foldl (\(a,l) x -> if all (\y -> arr!(y,x) == '.') [0..h] then (a+2, (a+2):l) else (a+1, (a+1):l)) (0,[]) [0..w]

getVPref :: GalArr -> [Int]
getVPref arr = let ((_, _), (h, w)) = bounds arr
               in reverse $ snd $ foldl (\(a,l) y -> if all (\x -> arr!(y,x) == '.') [0..w] then (a+2, (a+2):l) else (a+1, (a+1):l)) (0,[]) [0..h]
